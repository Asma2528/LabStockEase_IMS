...
import Model from './model.measuring';
import { GoPlus } from "react-icons/go";
import { useGetAllMeasuringItemsQuery } from '../../../provider/queries/Measuring.query';
import Loader from '../../../components/Loader';
import MeasuringCard from './Card.measuring';
import { useNavigate } from 'react-router-dom';
import { confirmDialog, ConfirmDialog } from 'primereact/confirmdialog';
import { toast } from 'sonner';
import { Button } from 'primereact/button';
import { useDeleteMeasuringItemMutation } from '../../../provider/queries/Measuring.query';
...
const MeasuringPage = () => {
    ...
    const { isLoading, data, isFetching } = useGetAllMeasuringItemsQuery({
        query: Search
    });
    ...
    const onSubmitHandler = (e) => {
        e.preventDefault();
        navigate(`/Measuring?query=${Search}`);
    };

    const [DeleteMeasuringItem] = useDeleteMeasuringItemMutation();
        ...
        footer: (
            <div className="p-dialog-footer">
                <Button 
                    ...
                    onClick={async () => {
                        try {
                            const { data, error } = await DeleteMeasuringItem(_id);
                            if (error) {
    ...
    return (
        ...
        <div className="w-full flex flex-wrap justify-evenly mt-10">
            <BreadCrumbs PageLink='/Measuring' PageName='Measuring' />
