...
import { useAddMeasuringItemMutation } from '../../../provider/queries/Measuring.query';

const Model = ({ visible, setVisible }) => {
    const [addMeasuringItem, { isLoading }] = useAddMeasuringItemMutation();
    ...
    const onSubmitHandler = async (values, { resetForm }) => {
        ...
        try {
            const response = await addMeasuringItem(values);
            if (response.error) {
                toast.error(response.error.data.message || 'Failed to add item');
                return;
            }
            toast.success("Measuring Item Added Successfully");
            resetForm();
            setVisible(false);
        } catch (e) {
