...
import { useLogIssuedQuantityMutation, useGetAllReagentsItemsQuery } from "../../../provider/queries/Reagents.query";
...
const Model = ({ visible, setVisible }) => {
  ...
  const { data: reagentsData, isLoading: isReagentsLoading, error } = useGetAllReagentsItemsQuery( {query: ''});

  // Handle error fetching reagents data
  if (error) {
    console.error("Error fetching reagents:", error);
  }

  // Log reagents data on change
  useEffect(() => {
    if (reagentsData) {
  ...
  const handleItemCodeChange = (e, setFieldValue) => {
    const selectedItem = reagentsData?.items?.find(
        ...
        {({ handleSubmit, setFieldValue }) => (
          <form onSubmit={handleSubmit} className="w-full">
            ...
            <Button
              ...
              disabled={isLoading || isReagentsLoading}
            >
