...
import Model from './log.model.reagents';
import ReagentsLogCard from './log.card.reagents';
import { useSearchReagentsLogsQuery } from "../../../provider/queries/Reagents.query";
import { useDeleteReagentsLogMutation } from '../../../provider/queries/Reagents.query';
import { confirmDialog, ConfirmDialog } from 'primereact/confirmdialog';
import { toast } from 'sonner';
import { Button } from 'primereact/button';

const ReagentsLogsPage = () => {
    ...
    const { data: logs, isLoading, isFetching } = useSearchReagentsLogsQuery(searchParams);
    const [deleteReagentsLog] = useDeleteReagentsLogMutation();
            ...
            footer: (
                <div className="p-dialog-footer">
                    <Button 
                        ...
                        onClick={async () => {
                            try {
                                const { data, error } = await deleteReagentsLog(_id);
                                if (error) {
    ...
    return (
        <>
            <div className="w-full flex flex-wrap justify-evenly mt-10">
                <BreadCrumbs PageLink='/Reagents' PageName='Reagent Log' />
    ...
    {isLoading || isFetching ? (
        ...
        logs.map(log => (
            <ReagentsLogCard key={log._id} data={log} onDelete={() => {
...
export default ReagentsLogsPage;