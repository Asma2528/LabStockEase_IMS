...
import { useAddReagantsItemMutation } from '../../../provider/queries/Reagants.query';

const Model = ({ visible, setVisible }) => {
    const [addReagantsItem, { isLoading }] = useAddReagantsItemMutation();
    ...
    const onSubmitHandler = async (values, { resetForm }) => {
        try {
            const response = await addReagantsItem(values);
            if (response.error) {
                toast.error(response.error.data.message || 'Failed to add item');
                return;
            }
            toast.success("Reagants Item Added Successfully");
            resetForm();
            setVisible(false);
        } catch (e) {
